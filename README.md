...
長文的全文頁會**依字數自動分頁**，每頁底部有「上一頁／下一頁」——因為 Kindle 是無 JS
的老 WebKit，分頁刻意做成一頁一個實體 HTML 檔，點「下一頁」是真正的整頁載入，正好觸發
墨水屏一次乾淨的**全頁刷新**，而不是滾動。每頁的字級／字體切換仍各自獨立。用 `config.yaml`
的 `page_chars` 調整每頁字數，設 `0` 則不分頁、維持單一滾動頁。
